...
class ControllerExtensionFeedSitemap extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('sitemap', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/feed/sitemap', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/feed/sitemap', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'] . '&type=feed', true);

		if (isset($this->request->post['sitemap_status'])) {
			$data['sitemap_status'] = $this->request->post['sitemap_status'];
		} else {
			$data['sitemap_status'] = $this->config->get('sitemap_status');
		}

		$data['data_feed'] = HTTP_CATALOG . 'index.php?route=extension/feed/sitemap';
